...
<style>
  ...
  .color-btn-muybien { background-color: #4afb4a; }
  .color-btn-bien { background-color: #a9ffa9; }
  .color-btn-normal { background-color: #f3f39d; }
  .color-btn-mal { background-color: #fd8697; }
  .color-btn-muymal { background-color: #ff3131; }
                  ...
                  <div class="icheck-inline">
                    @foreach($opciones as $idx => $opt)
                      ...
                      <div class="icheck-success d-inline">
                        <input type="radio" id="{{ $rid }}" name="{{ $pf['id'] }}" value="{{ $opt }}" required>
        ...
        <div class="card seg-list-card seg-accent-success shadow-sm mb-4">
          ...
          <div class="card-body">
                  ...
                  <div class="icheck-inline">
                    @foreach($opciones as $idx => $opt)
                      ...
                      <div class="icheck-info d-inline">
                        <input type="radio" id="{{ $rid }}" name="{{ $pp['id'] }}" value="{{ $opt }}">
                        <label for="{{ $rid }}">{{ $opt }}</label>
                      </div>
                    @endforeach
                  </div>
            ...
            <div class="d-flex justify-content-between mt-3">
              <button id="btn-prev" type="button" class="btn btn-secondary" style="border-radius: 8px; padding: 10px 20px; font-weight: 600;">
                Anterior
              </button>
...
<div class="color-picker-popover" id="colorPickerPopover">
  ...
  <div class="color-circle-btn color-btn-normal" data-value="normal" title="Normal"></div>
  <div class="color-circle-btn color-btn-mal" data-value="mal" title="Mal"></div>
  <div class="color-circle-btn color-btn-muymal" data-value="muymal" title="Muy Mal"></div>
</div>
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content border-0 shadow-lg" style="border-radius: 12px; overflow: hidden;">
      <div class="modal-header bg-danger text-white">
        <h5 class="modal-title font-weight-bold"><i class="fas fa-exclamation-triangle mr-2"></i>Respuestas Incompletas</h5>
      ...
      <div class="modal-body py-4">
        <p class="mb-0 text-muted" style="font-size: 1.05rem;">
          Por favor, responde todas las preguntas del cuestionario actual antes de continuar.
        </p>
      </div>
      <div class="modal-footer border-0">
        <button type="button" class="btn btn-secondary px-4 py-2 font-weight-bold" data-dismiss="modal" style="border-radius: 8px;">Aceptar</button>
      </div>
    </div>
  </div>
</div>

<div class="modal fade" id="modalSuccess" tabindex="-1" role="dialog" data-backdrop="static">
  <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
    <div class="modal-content border-0 shadow-lg" style="border-radius: 16px; overflow: hidden;">
      <div class="modal-body text-center p-5">
        <div class="mb-4">
          <i class="fas fa-heartbeat text-success" style="font-size: 4.5rem;"></i>
        </div>
        <h2 class="font-weight-bold mb-3" style="color: #0f172a;">Evaluación Médica Registrada</h2>
        <p class="text-muted mb-4" style="font-size: 1.1rem;">
          El reporte clínico ha sido guardado exitosamente. Se muestra el diagnóstico médico preventivo a continuación:
        </p>
        
        <div class="border rounded-lg p-4 text-left bg-light mb-4 shadow-sm" style="border-radius: 12px;">
          <div class="mb-3">
            <span class="badge badge-success px-3 py-2 text-uppercase font-weight-bold" style="font-size: 0.75rem; border-radius: 20px;">
              Estado Físico
            </span>
            <p class="mt-2 mb-0 font-weight-500" style="color: #334155; font-size: 1.05rem;" id="resFisicoText">
              -
            </p>
          </div>
          <div class="dropdown-divider my-3"></div>
          <div>
            <span class="badge badge-info px-3 py-2 text-uppercase font-weight-bold" style="font-size: 0.75rem; border-radius: 20px;">
              Estado Emocional
            </span>
            <p class="mt-2 mb-0 font-weight-500" style="color: #334155; font-size: 1.05rem;" id="resPsicoText">
              -
            </p>
          </div>
        </div>

        <button type="button" class="btn btn-primary btn-block py-3 font-weight-bold shadow-sm" data-dismiss="modal" onclick="resetSimulador()" style="border-radius: 10px; font-size: 1.1rem;">
          Finalizar y Volver
        </button>
      </div>
    </div>
...
<script>
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const colorClassByState = {
      muybien: 'estado-muybien',
      bien: 'estado-bien',
      normal: 'estado-normal',
      mal: 'estado-mal',
      muymal: 'estado-muymal'
    };
    const bodyState = {};
    ...
    function showPopover(x, y, svgEl, partName) {
      activePartElement = svgEl;
      activePartName = partName;
      
      // Marcar seleccionado
      const currentState = bodyState[partName];
      document.querySelectorAll('.color-circle-btn').forEach(btn => {
        btn.classList.remove('selected');
        if (btn.getAttribute('data-value') === currentState) {
    ...
    // Clicks en cuerpo
    document.querySelectorAll('#humanBody svg').forEach(svg => {
      svg.addEventListener('click', (e) => {
        e.stopPropagation();
        const partName = svg.getAttribute('data-part');
        const rect = svg.getBoundingClientRect();
        // Centrar tooltip sobre la pieza SVG usando coordenadas fijas (viewport)
        const left = rect.left + rect.width / 2;
        const top = rect.top;
        
        showPopover(left, top, svg, partName);
      });
    });

    // Seleccionar color en el popover
    document.querySelectorAll('.color-circle-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        if (!activePartElement || !activePartName) return;

        const val = btn.getAttribute('data-value');
        bodyState[activePartName] = val;

        // Limpiar clases anteriores y asignar la nueva
        activePartElement.classList.remove(...Object.values(colorClassByState));
        activePartElement.classList.add(colorClassByState[val]);
    ...
    nextBtn.addEventListener('click', () => {
      // Validar paso 1
      const pFisicas = ['f1','f2','f3','f4','f5','f6','f7','f8'];
      let complete = true;
      pFisicas.forEach(p => {
        if (!document.querySelector(`input[name="${p}"]:checked`)) {
          complete = false;
        }
      });
    ...
    sendBtn.addEventListener('click', () => {
      const pPsico = ['p1','p2','p3','p4','p5','p6','p7','p8'];
      let complete = true;
      pPsico.forEach(p => {
        if (!document.querySelector(`input[name="${p}"]:checked`)) {
          complete = false;
      ...
      // Procesar diagnóstico simulación
      let dolorPartes = [];
      for (let [part, state] of Object.entries(bodyState)) {
        if (state === 'mal' || state === 'muymal') {
          dolorPartes.push(part);
        }
      }

      let resFisico = '';
      if (dolorPartes.length > 0) {
        resFisico = `Se registraron molestias críticas o lesiones en: **${dolorPartes.join(', ')}**. Requiere atención médica para descartar quemaduras o problemas musculares post-incendio.`;
      } else {
        resFisico = 'Óptima salud física. Sin molestias ni signos de quemadura reportados en el mapa corporal interactivo.';
      }

      let resPsico = 'Se detectaron síntomas leves de fatiga adaptativa. Se aconseja descanso de 48 horas y sesión preventiva de apoyo emocional con el psicólogo de brigada.';

      document.getElementById('resFisicoText').innerHTML = resFisico;
      document.getElementById('resPsicoText').innerHTML = resPsico;
  ...
  function resetSimulador() {
    ...
    document.querySelectorAll('#humanBody svg').forEach(svg => {
      svg.classList.remove('estado-muybien', 'estado-bien', 'estado-normal', 'estado-mal', 'estado-muymal');
    });
    document.getElementById('step-psico').classList.add('step-hidden');
    document.getElementById('step-fisico').classList.remove('step-hidden');
    window.scrollTo({ top: 0, behavior: 'smooth' });
  }
</script>
@endpush
