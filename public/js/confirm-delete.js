(function () {
    ...
    function buildModal() {
        ...
        m.innerHTML = '<div class="modal-dialog modal-sm modal-dialog-centered" role="document">'
            ...
            + '<div class="modal-body"><p class="mb-0">¿Estás seguro de que deseas eliminar este registro? Esta acción no se puede deshacer.</p></div>'
    ...
    document.addEventListener('click', function (e) {
        ...
        if (e.target.closest('.js-cancel') || e.target.closest('#confirmDeleteModal .close')) {
            e.preventDefault();
            closeModal();
            return;
        }

        if (e.target.closest('#confirmDeleteModal .js-confirm')) {
            e.preventDefault();
            var f = targetForm;
            closeModal();
            if (f) {
