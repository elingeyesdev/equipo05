...
@section('content')
    ...
    <div class="info-box">
        ...
        <p><strong>TelÃ©fono:</strong> {{ $rescuer->person->telefono ?? 'N/A' }}</p>
        @if($rescuer->motivo_postulacion)
            <p><strong>Motivo de postulaciÃ³n:</strong> {{ $rescuer->motivo_postulacion }}</p>
        @endif
    ...
    <p>
        <a href="{{ route('rescate.rescuers.show', $rescuer->id) }}" class="button">Ver Detalles de la Solicitud</a>
    </p>
@endsection
