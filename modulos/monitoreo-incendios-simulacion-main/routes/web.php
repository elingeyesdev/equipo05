...
Route::middleware(['auth', 'permission.check:'.FusionModuloAccess::INCENDIOS_PERMISSIONS])->group(function () {
    ...
    // Monitoreo público: mapa, clima y datos de apoyo (ciudadanos + operadores)
    Route::middleware(['permission.check:'.FusionModuloAccess::INCENDIOS_CITIZEN_PERMISSIONS])->group(function () {
    ...
    // Reporte ciudadano de focos (sin acceso al panel operativo completo)
    Route::middleware(['permission.check:ciudadano.incendios.reportar|incendios.focos.gestionar'])->group(function () {
        Route::get('focos-incendios/create', [Modules\Incendios\Http\Controllers\FocosIncendioController::class, 'create'])
            ->name('focos-incendios.create');
        Route::post('focos-incendios', [Modules\Incendios\Http\Controllers\FocosIncendioController::class, 'store'])
            ->name('focos-incendios.store');
    });

    // Panel operativo: gestión, reportes internos y CRUD
    Route::middleware(['permission.check:'.FusionModuloAccess::INCENDIOS_OPERATE_PERMISSIONS])->group(function () {
